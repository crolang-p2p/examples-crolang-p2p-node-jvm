...
fun main(){
    ...
    val bobAsyncCallbacks = AsyncCrolangNodeCallbacks(
    ...
    val carolAsyncCallbacks = AsyncCrolangNodeCallbacks(
        onConnectionSuccess = { node ->
            println("Connected to Node ${node.id} successfully")
            node.send("GREETINGS_CHANNEL", "Hello there!")
        },
    ...
    CrolangP2P.Kotlin.connectToBroker(BROKER_ADDR, ALICE_ID)
        .onSuccess {
            ...
            CrolangP2P.Kotlin.connectToMultipleNodesAsync(
                targets = mapOf(
                    BOB_ID to bobAsyncCallbacks,
                    CAROL_ID to carolAsyncCallbacks
                ),
                onConnectionAttemptConcluded = {
                    println("All connection attempts concluded")
                    it.forEach{ entry ->
                        entry.value.onSuccess {
                            println("Connected to Node ${entry.key} successfully")
                        }.onFailure { reason ->
                            println("Failed to connect to Node ${entry.key}: $reason")
                        }
                    }
                },
            )
